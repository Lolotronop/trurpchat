// copied from https://github.com/polvallverdu/stick-to-bottom-svelte
/*!---------------------------------------------------------------------------------------------
  ...
  scrollToBottom = (
    ...
    const next = async (): Promise<boolean> => {
      const promise = new Promise(requestAnimationFrame).then(() => {
        ...
        const tickDelta =
          (tick - (this.lastTick ?? tick)) / SIXTY_FPS_INTERVAL_MS;
        ...
        if (
          snapshottedScrollTop <
          Math.min(startTarget, this.calculatedTargetScrollTop)
        ) {
          if (isEqual($state.snapshot(this.animation.behavior), behavior)) {
            if (behavior === "instant") {
            ...
            this.velocity =
              (behavior.damping * this.velocity +
