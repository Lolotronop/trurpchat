<script lang="ts">
  ...
  function setAudioMaxInSDP(
    ...
    for (const line of lines) {
      ...
      const videoMatch = line.match(
        /^a=rtpmap:(\d+)\s+(H264|VP8|VP9|AV1)\/90000/i,
      );
      if (videoMatch) {
  ...
  async function createInlineNoiseGate(
    context: AudioContext,
    { threshold = -50, attackTime = 0.01, releaseTime = 0.2 } = {},
  ) {
  ...
  let localLimiterNode: DynamicsCompressorNode =
    localAudioContext.createDynamicsCompressor();
  ...
  let localNoiseGate: AudioWorkletNode = $state.raw(undefined);
  createInlineNoiseGate(localAudioContext).then((v) => {
    ...
    localNoiseGate.port.onmessage = (e: MessageEvent) => {
      const { isOpen } = e.data;
  ...
  async function setAudioInputDevice(deviceId?: string) {
    ...
    if (deviceId) {
      (settings.audio as MediaTrackConstraints).deviceId = { ideal: deviceId };
    }
    localStream = await navigator.mediaDevices.getUserMedia(settings);
    if (localSourceNode) {
    ...
    localAnalyserNode.connect(localNoiseGate);
    localNoiseGate.connect(localDestination);
  ...
  async function joinRoom() {
    if (!socket || !clientId || !roomToJoin.trim() || !username.trim()) return;
  ...
  async function handleSignalingMessage(event: MessageEvent) {
    ...
    switch (data.type) {
      ...
      case "room-joined":
        ...
        console.log("Joined room:", currentRoom, "with users:", remoteUsers);
  ...
  function getUserDisplayName(user: { id: string; username?: string }): string {
      ...
      update(newData: { stream: MediaStream; userId: string }) {
        if (element && newData.stream) {
          ...
          try {
            ...
            const source = audioContext.createMediaStreamSource(newData.stream);
            const gainNode = audioContext.createGain();
      ...
      {:else}
        ...
        <div>
          <div style={`height: 50px; background-color: #f0f0f0; width: 500px`}>
            <div
              style={`position: absolute; height: 50px; background-color: #FF0000; width: ${(localLoudnessPeakLevel * 500) / 60}px; transition: all 0.04s ease;`}
            ></div>
            <div
              style={`position: absolute; height: 50px; background-color: #00FF00; width: ${(localLoudnessLevel * 500) / 60}px; transition: all 0.04s ease;`}
            ></div>
          </div>
            ...
            <input
              ...
              bind:value={
                ...
                (v) => {
                  ...
                  const param =
                    localNoiseGate?.gateNode?.parameters?.get("threshold");
          ...
          <div
            style={`height: 30px; background-color: #f0f0f0; width: 200px; margin-top: 10px; display: flex; align-items: center; justify-content: center; border-radius: 4px;`}
          >
            <div
              style={`height: 20px; background-color: ${localGateState > 0.5 ? "#00FF00" : "#FF0000"}; border-radius: 2px; transition: all 0.1s ease; width: 100%;`}
            ></div>
            <span
              style="position: absolute; color: ${localGateState > 0.5
                ? '#000'
                : '#fff'}; font-size: 12px; font-weight: bold;">GATE</span
            >
          </div>
          ...
          {#each availableAudioDevices as device (device.deviceId)}
            <button onclick={() => setAudioInputDevice(device.deviceId)}>
              {device.label}
              {device.deviceId === selectedAudioDeviceId ? "Selected" : ""}
            </button>
          {/each}
        </div>
      {/if}
    ...
    <div class="video-grid">
      ...
      {#each Array.from(remoteStreams.entries()) as [userId, stream] (userId)}
        <div class="video-container remote">
          ...
          <!-- svelte-ignore a11y_media_has_caption -->
          <video autoplay use:setVideoStream={{ stream, userId }}></video>
          <div class="volume-control">
            <span class="volume-icon"
              >{getVolumeIcon(userVolumes.get(userId) || 1.0)}</span
            >
            <input
              type="range"
              min="0"
            ...
            <span class="volume-value"
              >{Math.round((userVolumes.get(userId) ?? 1.0) * 100)}%</span
            >
          </div>
        </div>
      {/each}
